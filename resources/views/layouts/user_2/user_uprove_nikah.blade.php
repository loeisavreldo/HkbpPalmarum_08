      ...
      <div class="judul_1" id="targetElement">
        <div class="">
          <table table class="aesthetic-table-user" >
            <thead>
              <tr>
                <th  style=" font-weight:40px ;border: none; background-color: #37545f ; color:white; padding:40px;  "colspan="3">Status Pendaftaran Pernikahan</th>
              </tr>
              <tr>
                <th class="judul-tabel">Nama Kedua Mempelai</th>
                <th class="judul-tabel">Status</th>
              </tr>
            </thead>
            @foreach($userNikahs as $data)
            @if($data['status']== 0)
            <tbody class="info-3">
              <tr>
                <td class="lebar">{{$data['nama_lengkap_laki']}}&nbsp<span style="color: #e25555;">&#9829;</span>&nbsp{{$data['nama_lengkap_perempuan']}}</td>
                <td>
                  @if($data['status']== 0)
